...
/**
 * 오늘 보여줄 단어들. 첫 번째가 "오늘의 단어" 고 나머지는 홈이 그 아래에
 * 같이 보여주는 것들이다.
 *
 ...
 * 목록 끝에 걸린 날에만 짧아진다.
 *
 * 다만 하루 안에 안 바뀐다는 보장은 아니다. 검수가 하나 통과되거나 취소되면
 * count 가 바뀌는데, 시작점이 `day % count` 라 나누는 수가 바뀌면 시작점이
 * 통째로 옮겨 간다 - 한 칸이 아니라 대략 day / count 칸(지금 약 36칸)이다.
 * 그날은 오늘의 단어도, 그 아래 목록도 전날 예상과 다르다. 정말 고정하려면
 * 백엔드에 daily 엔드포인트가 필요한데, 지금은 그만한 값어치가 없다.
 *
