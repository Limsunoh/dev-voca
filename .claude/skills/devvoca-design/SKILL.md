...
## 모바일 앱 — 방향은 미정, 웹에 Ionic 은 넣지 않는다

앱으로 하려는 것은 1일 1학습 푸시, 학습 루틴(단어 → 문장 → 문제풀이), 문제풀이 속도·점수, 전 유저 랭킹, 스트릭이다.

**이 중 클라이언트가 필요한 건 푸시 하나뿐이다.** 점수·속도 측정·랭킹·스트릭은 전부 백엔드 기능이고 웹에서도 똑같이 동작한다. 그래서 **학습 기록과 점수 API 를 먼저 만들고, 클라이언트(PWA / React Native)는 그다음에 고른다.** 지금 프레임워크를 문서에 박아두면 정작 만들 것이 서버 쪽인데 앱 껍데기부터 세우게 된다.

`@ionic/cli` 는 전역에 설치돼 있지만(`ionic --version`), **Ionic 채택은 접었다.** Ionic 의 장점은 웹 자산 재사용인데 앱과 웹은 UI 를 공유하지 않기로 했으니 그 장점이 사라지고, WebView 라는 단점만 남는다. Apple 가이드라인 4.2 는 2026 년 현재 웹 래퍼를 사실상 거절하며, 푸시 같은 API 몇 개를 붙이는 것으로는 부족하다고 명시한다. 네이티브 UI 로 컴파일되는 React Native / Flutter 는 이 문제를 피한다.

**웹에 Ionic 컴포넌트를 넣지 않는다.** 이건 확정이다. 취향 문제가 아니라 맞물리지 않는다.
