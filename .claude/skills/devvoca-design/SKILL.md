...
## 모바일 앱은 따로 만든다 (웹에 Ionic 을 섞지 않는다)

devvoca 를 iOS/안드로이드 앱으로도 낼 계획이다. 경로는 **Ionic + Capacitor 별도 프로젝트**이고, 지금 이 Next.js 웹은 구조를 그대로 둔다. `@ionic/cli` 는 전역에 설치돼 있다(`ionic --version` 으로 확인).

**웹에 Ionic 컴포넌트를 넣지 않는다.** 취향 문제가 아니라 맞물리지 않는다.
...
앱 작업을 시작할 때는 이 문서가 아니라 별도 프로젝트에서 다룬다. 여기 규칙은 **웹**에 적용된다.
