...
무엇을 "먼저 달성" 으로 볼지는 순위표마다 다르다.
    ...
    꾸준함     - **먼저 달성으로 가르지 않는다.** 그 날 수에 도달한
                 순간은 점수를 얻은 마지막 날이라 구하기는 쉽지만, 그걸
                 위로 두면 일찍 그만둔 사람이 지금도 하는 사람을 이긴다.
                 대신 **점수 합이 많은 쪽**을 위로 둔다. 같은 사흘이어도
                 그 사흘에 더 많이 푼 쪽이다.

## 꾸준함은 날 수로 매긴다

점수 합으로 매기던 때는 화면 설명("점수를 얻은 날을 모두 더합니다")과
순위가 달랐다.
하루 점수에 상한이 있어도, 이틀 몰아친 사람이 매일 조금씩 한 사람을
이겼다. 이름이 꾸준함이면 날 수가 먼저다. 점수 합은 동점을 가를 때만
본다.
"""
...
# "점수가 남은 날" 이면 1, 아니면 0. 꾸준함의 날 수는 이걸 더한 것이다.
# 판을 열기만 해도 그 날 행이 생기므로 행 수로 세면 망친 날까지 들어간다.
#
# **두 칸을 따로 보면 안 된다.** best=5, study=-5 인 날은 총점이 0인데
# 두 칸 중 하나가 양수라 날 수에 들어가 버린다. 점수를 1점도 안 준 날을
# 여럿 쌓아 순위를 올리는 길이 열린다 - 막으려던 바로 그것이다.
...
class Row:
    ...
    # 로그인 안 한 사람에게 나간다.
    avatar: dict[str, str]
    # 순위를 매기는 값. 최고점은 한 판 점수, 꾸준함은 점수를 얻은 날 수다.
    score: int
    # 곁들이는 값. 최고점은 판 수, 꾸준함은 점수 합이다.
    entries: int
...
def _best_by_user(rounds: QuerySet) -> QuerySet:
    ...
    안 거르면 0일이 꼬리에 쌓이지만, 여기 한 줄은 **판을 실제로 끝내야**
...
def _streak_by_user(days: QuerySet, *, only_scored: bool = True) -> QuerySet:
    """사용자별 꾸준함. 점수를 얻은 날 수(score)와 점수 합(entries).

    score 에 날 수를 담는 이유: 세 순위표 모두 **score 가 순위를 매기는
    값**이다. 화면은 score 를 크게, entries 를 작게 그린다.
    꾸준함만 순서가 다른 칸에 있으면 화면과 _streak_ahead 가 종류를
    알아야 한다.
    ...
    rows = (
        ...
        .annotate(
            ...
            # 행 수로 세면 망친 날도 들어간다. 그러면 망친 날을 쌓은 사람이
            # 위로 올라가 - 꾸준함이 아니라 "행을 많이 만든 사람" 이 이긴다.
            score=Sum(_DAY_COUNTS),
            # 날 수가 같으면 점수 합이 많은 쪽이 위.
            entries=Sum("_day_total"),
        )
    )

    if only_scored:
        # 점수를 얻은 날이 없는 사람은 뺀다. 그냥 두면 한 문제도 안 푼
        # 사람들이 0일 동점으로 꼬리에 쌓인다. 하루 점수는 0 밑으로 안
        # 가므로 "0일" 과 "합 0점" 은 같은 사람들이다.
...
def _streak_rows_for(user) -> list[dict]:
    ...
    0일도 낸다. 목록에서는 0일을 빼지만 "내가 몇 등인가" 는 다른 질문이다 -
...
def _streak_ahead(item: dict, user) -> int:
    ...
    """
    # 0일도 포함해서 센다. 목록은 0일을 빼지만 여기서까지 빼면 score=0
    # 가지가 죽어, **0일인 사람들이 전부 같은 등수**가 된다. 목록에 안
    # 보이는 사람들이라 눈에 안 띈다.
