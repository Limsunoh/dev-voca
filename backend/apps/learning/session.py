...
# 부풀릴 여지가 없다.
#
# 화면의 판정 연출이 1000ms 다(globals.css 의 --duration-verdict). 같은
# 값으로 둔다.
#
# **더 작으면 안 된다.** 연출은 66% 에 부장님이 학생에게 닿고, 74~92% 에
# 판정 글자가 떴다 사라진다. 서버가 900 을 주면 그 글자 구간이 잘려서
# 맞았는지 틀렸는지를 못 보고 다음 문제가 뜬다.
#
# 전에는 연출이 600ms 였고 여기가 800 이었다 - 도착한 자세를 볼 틈으로
# 200ms 를 더 뒀다. 연출을 디자인 원본으로 되돌리며 1000ms 가 되었고,
# 그 틈은 이제 연출 안에 들어 있다(78~100% 가 어둠이 걷히는 구간이다).
#
# **화면도 이 값을 써야 한다.** 서버가 미루는 양과 화면이 멈추는 시간이
# 다르면 차이만큼 매 문제 쌓인다. 그래서 판을 열 때 이 값을 함께 내려주고
# (views.RoundStartView) 화면은 받은 값으로 멈춘다.
REACTION_PAUSE_MS = 1000
...
def _issue(state: dict, now: datetime, shown_after_ms: int = 0) -> dict | None:
    ...
    제한 시간이 깎인다.** 3초짜리 문제에서 1000ms 면 창이 2초로 줄고,
