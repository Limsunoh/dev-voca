...
# (term, meaning, category, difficulty, description, example, example_translation)
WORDS: list[tuple[str, str, str, int, str, str, str]] = [
    # ---------- git ----------
    (
        "commit", "변경사항을 저장하다", "git", E,
    ...
    (
        "deprecated", "더 이상 권장하지 않는", "review", N,
    ...
    (
        "edge case", "경계에 놓인 예외적 경우", "review", N,
    ...
    (
        "boilerplate", "매번 반복되는 형식적인 코드", "review", N,
    ...
    (
        "serialize", "전송할 수 있는 형태로 바꾸다", "api", N,
    ...
    (
        "rollback", "작업 전체를 무르다", "database", N,
    ...
    (
        "fallback", "안 될 때를 위한 대비책", "devops", N,
    ...
    # ---------- 디버깅 ----------
    (
        "stack trace", "에러가 난 경로 기록", "debug", E,
    ...
    (
        "reproduce", "같은 문제를 다시 일으키다", "debug", N,
    ...
    (
        "race condition", "순서가 꼬여 생기는 오류", "debug", H,
    ...
    (
        "regression", "고쳐졌던 것이 다시 깨짐", "debug", N,
