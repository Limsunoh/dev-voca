...
class Question:
    ...
    #
    # **검수 게이트의 입력이기도 하다.** 일일공부는 낸 문제를 저장해 두고
    # 이어 풀 때 다시 주는데, 그때 보기와 이 문장이 아직 검수된 상태인지
    # 다시 거른다(daily_study._hidden_reason). 보기에 없는 콘텐츠로 지문을
    # 만드는 유형을 새로 만들면 반드시 이 칸을 채워야 한다 - 안 채우면 그
...
def make_for_word(answer: Word, pool: QuerySet[Word], kind: str | None = None) -> Question | None:
    ...
    복습과 문제풀기의 ?item= 이 쓴다 - 둘 다 무엇을 낼지 이미 정해져
    있다. 복습에서 무작위로 고르면 복습이 아니라 그냥 문제풀이가 되고,
    상세에서 "이 단어로 문제 풀기" 를 눌렀는데 다른 단어가 나오면 안 된다.
